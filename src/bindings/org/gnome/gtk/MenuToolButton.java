 ...
 * 
 * <p>
 * Near the Button itself, a MenuToolButton shows another little Button with
 * an arrow. When the user clicks this additional Button, a dropdown Menu is
 * popped up.
 * 
 * <p>
 ...
 * your application can have a MenuToolButton for the <i>New Document</i>
 * action, using the attached Menu to let users choose what kind of new
 * document they want to create.
 * 
 * <p>
 * A MenuToolButton uses to have a default action, to be executed when user
 * clicks the Button itself, and not the the arrow Button. You can capture
 * that event with the {@link ToolButton.CLICKED CLICKED} signal. User Menu
...
public class MenuToolButton extends ToolButton
{
     ...
     * Label and Image as other GNOME applications. To ensure consistent
     * look-n-feel between applications, it is highly recommend that you use
     * provided stock item whenever possible.
     * 
     * @param stockId
     *            The StockId that will determine the Label and icon of the
     *            MenuToolButton.
     */
    public MenuToolButton(StockId stockId) {
        super(GtkMenuToolButton.createMenuToolButtonFromStock(stockId.getStockId()));
    }
     ...
     * supported until we change Plumbing.pointerOf
     */
     ...
     * 
     * @return
     *      The associated Menu or <code>null</code> if no Menu has been set.
     */
    public Menu getMenu() {
