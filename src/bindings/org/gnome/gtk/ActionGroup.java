 ...
 * documents, one group holding global Actions (e.g. quit, about, new), and
 * one group per document holding Actions that act on that document (eg. save,
 * cut/copy/paste, etc). Each Window's Menus would be constructed from a
 * combination of two ActionGroups.
 * 
...
public class ActionGroup extends Object
{
     ...
     * sensitive to really allow users to activate some Action. See
     * {@link Action#setSensitive(boolean) Action.setSensitive()} for further
     * details.
     */
    ...
    /**
     * Get if this ActionGroup is sensitive, i.e., if its Actions can be
     * activated by users. Take care that even if the ActionGroup is
     * sensitive, any of the Actions could not be.
     */
     ...
     * {@link Action#setVisible(boolean) Action.setVisible()} for further
     * details.
     */
     ...
     * the ActionGroup is visible, any of the Actions could not be.
     */
    ...
    /*
     * FIXME
     * Gtk+ docs claim that this function can lead to problems if the user
     * tries to modify the accelerator of a MenuItem associated with the Action.
     * Thus, I'm tempted to map it to GtkActionGroup.addActionWithAccel,
     * with a "" accelerator. 
     * Anyway, given that accelerators are not exposed yet, I will delay this
     * discussion a bit.
     */
