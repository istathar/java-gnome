...
public class TextBuffer extends Object
{
     ...
     * TODO (SP) I don't see any advantage in using the the mark_names,
     *   unless you need to access built-in names like "insert" or
     *   "selection-bound" (are there more?)
     */
    ...
    /**
     * Insert a text at a given position. All {@link TextIter} behind
     * the position move accordingly, while marks keep their position.
     */
    /*
     * TODO (SP) Is there any point in exposing the length param?
     */
     ...
     * non-editable location in the buffer - meaning that it is enclosed
     * in TextTags that mark the area non-editable.<br/>
     * 
     * @param pos   Position to insert at. 
     * @param text  Text to insert.
     * @param defaultEditable  How shall the area be handled, if there
     *    are no tags affacting editability.
     */
    ...
    /**
     * Returns the current cursor position. Is also used at the start
     * position of a selected text. 
     * 
    ...
    /**
     * Returns the end of the current selection. If you need the beginning
     * of the selection use {@link #getInsert() getInsert()}.
     * 
     * You can call {@link #getIterAtMark(TextMark) getIterAtMark()} to
     * convert the TextMark to an TextIter.
     */
    /*
     * (SP) I tested around a bit and getSelectionBound() always returned
     * the same position as getInsert(). I checked the generated code -
     * it looks ok. A bug in GTK ?
     * This even is true if I access the mark "selection_bound" directly.
     */
     ...
     * 
     * @return A array of two TextIter (start,end) marking the selection
     * or <code>null</code> if nothing was selected.
     */
    ...
    /**
     * Apply the selected tag on the area in the TextBuffer between the
    ...
    /*
     * TODO (SP) When we don't expose mark names we also don't need to
     * expose tag names, don't we?
     */
